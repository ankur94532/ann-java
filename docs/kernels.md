...
The tables and every figure in the prose below are generated by `scripts/readme_tables.py`
from committed JMH output: [`jmh-distance.json`](results/jmh-distance.json) is the full run,
and [`jmh-scan.json`](results/jmh-scan.json) and
[`jmh-scanram.json`](results/jmh-scanram.json) are re-runs of the block-scan benchmarks with a
4 GiB heap, layered over it in that order. To take a new run and preview its tables:
...
<!-- END GENERATED: kernel-tables -->

## What the numbers say

<!-- BEGIN GENERATED: kernel-findings -->
...
over. The fixed cost is about 4 ns of the 13.3 ns call: the 32 steps themselves, at the
d=960 per-step cost below, account for only 9.4.
...
roughly 13 cycles for what is 4 multiplies and 4 adds of real work. (Cycle counts in this
section take the clock as about 4.4 GHz.)
...
close to the machine's load-issue limit of two 128-bit loads per step. So the 10.2x is two
separate wins stacked — 3.9x from the lanes, ~2.6x from breaking a serial dependency that
...
75.2 M vec/s at d=128, matching the 75.2 M vec/s the L1-resident pair benchmark
...
At d=960 that stops being quite true. The SIMD kernel manages 14.2 M vec/s on an
L1-resident pair but 12.9 M vec/s scanning from DRAM — a 9% loss while moving
49.6 GB/s — so at 960 dimensions the kernel has become fast enough to start feeling the
